...
def register_account(request):
    if request.method == 'POST':
        ...
        cursor.execute(
            ...
            (user_acct_id, perm_id, username, email, gym_id, firstname, lastname, datejoined, hashed_password)
...
def login_view(request):
    if request.method == 'POST':
        ...
        try:
            ...
            cursor.execute("SELECT user_acct_id, password_val, perm_id FROM user_accounts WHERE username = ?", (username,))
            user = cursor.fetchone()

            if user:
                user_id, stored_password_hash, perm_id = user
...
def home_owner(request):
    ...
    try:
        ...
        cursor.execute("SELECT username, perm_id, gym_id FROM user_accounts WHERE user_acct_id = ?", (user_id,))
        user = cursor.fetchone()

        if user:
            username, perm_id, user_gym_id = user  # Get the gym_id here
...
def home_staff(request):
    ...
    try:
        ...
        cursor.execute("SELECT username, perm_id, gym_id, first_name, last_name FROM user_accounts WHERE user_acct_id = ?", (user_id,))
        user = cursor.fetchone()

        if user:
            username, perm_id, gym_id, first_name, last_name = user
